...
입력
같은 날 동시에 가입한 인원 3명이 규칙적으로 방문하는, 방문주기가 공백을 두고 입력된다.
(단, 입력값은 100이하의 자연수이다.)

출력
3명이 다시 모두 함꼐 방문해 문제를 풀어보는날(동시 가입/등업 후 며칠 후?)을 출력한다.
"""

person1_cycle, person2_cycle, person3_cycle = map(int, input().split())

day = 1

while(
    day % person1_cycle != 0
    or day % person2_cycle != 0
    or day % person3_cycle != 0
):
    day += 1
